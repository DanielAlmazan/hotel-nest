...
const cargarDatos = async () => {
  ...
  for (let i = 0; i < habitaciones.length; i++) await habitaciones[i].save();
  for (let i = 0; i < limpiezas.length; i++) await limpiezas[i].save();
  for (let i = 0; i < usuarios.length; i++) await usuarios[i].save();
};
